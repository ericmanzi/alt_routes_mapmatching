...
def executeRouteMapMatchService(route)
	@mm = MapMatcher.new
	@route = route
...
alternate_routes = ActiveRecord::Base.connection.select_all("select * from alternate_routes;").to_a

alternate_routes[0..3].each do |route| 
	Delayed::Job.enqueue executeRouteMapMatchService(route), :queue => "general"
end
