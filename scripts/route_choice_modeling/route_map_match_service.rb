class RouteMapMatchService
  ...
  private
      def map_match
        ...
        begin
          ...
          @segments.each_with_index do |s, index|
                segments.push "('#{s.start_time}', '#{s.end_time}', #{s.edge_id}, #{s.osm_way_id}, '#{s.try(:name).try(:gsub, "'", "")}', #{s.source_id}, #{s.target_id}, '#{s.geom_way}', #{@route.user_id}, #{index+1}, '#{DateTime.now}', '#{DateTime.now}', #{s.mph*0.621371}, #{s.clazz}, #{s.flags})"
          end
      ...
      def emulate_readings
        ...
        points = @decoder.decode(@route.polyline)
      ...
      def extract_polyline
        ...
        @route.map_matched_polyline = Polylines::Encoder.encode_points(points)
      end
